...
# Script custom pentru testare XSS
...
for IP in $LIVE_HOSTS; do
    	echo "[***] ($COUNTER) Scanez $IP pentru vulnerabilități de tip XSS (Cross-Site Scripting)..."
    	sudo nmap -A -sV --script "http-dombased-xss,http-stored-xss,http-phpself-xss,http-vuln-cve2013-6786" "$IP" -oN "NMAP_output/$OUTPUT_DIR/scan_$IP.txt"
    	((COUNTER++))
done
