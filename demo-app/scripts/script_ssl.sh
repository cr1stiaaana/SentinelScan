...
# Script custom pentru testare SSL
...
for IP in $LIVE_HOSTS; do
    	echo "[***] ($COUNTER) Scanez $IP pentru vulnerabilități de tip SSL..."
    	sudo nmap -A -sV -script=ssl*  "$IP" -oN "NMAP_output/$OUTPUT_DIR/scan_$IP.txt"
    	((COUNTER++))
done
